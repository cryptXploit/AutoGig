              ...
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-3 gap-4">
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                    ...
                    <div className={\`text-lg font-bold \${data.applicationIntelligence.recommendation === 'APPLY' ? 'text-emerald-600' : data.applicationIntelligence.recommendation === 'SKIP' ? 'text-red-600' : 'text-amber-600'}\`}>
                    ...
                    {data.applicationIntelligence.tailoredResume?.map((skill: any, idx: number) => (
                      <div key={idx} className="p-3 border border-slate-200 rounded-lg flex flex-col justify-between">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <span className={\`text-[10px] font-bold px-2 py-1 rounded-full uppercase \${skill.relevance === 'HIGH' ? 'bg-blue-100 text-blue-700' : skill.relevance === 'MEDIUM' ? 'bg-slate-100 text-slate-700' : 'bg-slate-50 text-slate-400'}\`}>
                            {skill.relevance}
                          </span>
                        </div>
                        <div className="text-xs text-slate-600 mb-2">{skill.reason}</div>
                        <div className="flex items-center gap-2 mt-auto">
                          <span className={\`text-[10px] font-bold uppercase px-2 py-1 rounded \${skill.matchType === 'DIRECT_MATCH' ? 'bg-emerald-100 text-emerald-700' : skill.matchType === 'RELATED_MATCH' ? 'bg-blue-100 text-blue-700' : skill.matchType === 'EVIDENCE_WEAK' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}\`}>
                            {skill.matchType}
                          </span>
