    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 pb-32">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div>
          ...
          <div className="text-sm text-slate-500 mt-1">{opportunity.client} • {opportunity.url}</div>
        </div>
                ...
                <div className="p-6">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                      ...
                      <div className={\`text-lg font-bold \${evaluation.recommendedAction === 'RECOMMEND' ? 'text-emerald-600' : evaluation.recommendedAction === 'COUNTER' ? 'text-amber-600' : 'text-red-600'}\`}>
                        {evaluation.recommendedAction || evaluation.route || 'UNKNOWN'}
                      </div>
                    </div>
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="bg-emerald-50/50 p-4 rounded-xl border border-emerald-100">
                        <div className="text-xs font-bold text-emerald-800 uppercase mb-2 flex items-center gap-1"><CheckCircle className="w-3 h-3"/> Positive Signals</div>
                        <p className="text-sm text-emerald-900 leading-relaxed">{evaluation.opportunityAssessment || 'No assessment provided.'}</p>
                      </div>
                      <div className="bg-amber-50/50 p-4 rounded-xl border border-amber-100">
                        <div className="text-xs font-bold text-amber-800 uppercase mb-2 flex items-center gap-1"><AlertTriangle className="w-3 h-3"/> Caveats / Risks</div>
                        <p className="text-sm text-amber-900 leading-relaxed">
                          {evaluation.risks && evaluation.risks.length > 0 ? (
                            <ul className="list-disc pl-4">{evaluation.risks.map((r: string, i: number) => <li key={i}>{r}</li>)}</ul>
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 mt-4">
                      <div className="text-xs font-bold text-slate-500 uppercase mb-2">Agent Final Conclusion</div>
                      <p className="text-sm text-slate-800 italic leading-relaxed">"{evaluation.reasoningSummary || evaluation.explanations || 'Agent concluded processing.'}"</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div className="p-6">
                  <div className="flex flex-wrap gap-6 mb-4">
                    ...
                    <div>
                      <div className="text-xs text-slate-400 font-bold uppercase">Your Min Rate</div>
                      <div className="text-xl font-bold text-slate-600">${preferences?.minRate || '--'}</div>
                    </div>
                    <div>
                      <div className="text-xs text-slate-400 font-bold uppercase">Your Target Rate</div>
                      <div className="text-xl font-bold text-emerald-600">${preferences?.targetRate || '--'}</div>
                    </div>
                    <div>
                  ...
                  <p className="text-sm text-slate-700 bg-slate-50 p-3 rounded-lg border border-slate-100">
                    {evaluation.economicAssessment || 'No deep economic analysis available.'}
                  </p>
                </div>
              </div>
            </>
...
`;
fs.writeFileSync('apps/web-app/app/opportunity/[id]/page.tsx', code, 'utf-8');
