...
const { EventBusImpl } = require('../packages/events/dist');
...
db.exec(\`
  CREATE TABLE opportunities (id TEXT PRIMARY KEY, source TEXT, sourceJobId TEXT, canonicalUrl TEXT, title TEXT, description TEXT, normalizedSkills TEXT, normalizedBudget REAL, deadline TEXT, client TEXT, provenance TEXT, sourceReliability REAL, publishedAt TEXT, ingestionTimestamp TEXT, status TEXT);
...
async function testPhaseF() {
  ...
  for (const c of ssrfCases) {
    await t(\`SSRF: \${c.name}\`, async () => {
      ...
      try {
        await (src as any).fetchSafe(c.url);
  ...
  const baseOpp = { source: 's1', sourceJobId: 'j1', canonicalUrl: 'http://a.com', title: 'T1', description: 'desc', skills: [], publishedAt: new Date().toISOString() };
  ...
  await t('Pipeline: uncertain duplicate (semantic)', async () => {
    await pipeline.process({ ...baseOpp, sourceJobId: 'j3', canonicalUrl: 'http://c.com' }, 's1');
    const opp = db.prepare('SELECT * FROM opportunities WHERE sourceJobId = ?').get('j3');
    if (!opp) throw new Error('Did not save uncertain dup');
    if (!opp.status || !opp.status.includes('DISCOVERED')) throw new Error('Missing status');
...
`;
fs.writeFileSync('tests/phase-f.test.js', content);
